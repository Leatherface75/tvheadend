...
tvh_video_context_wrap(TVHContext *self, AVPacket *avpkt, th_pkt_t *pkt)
{
    ...
    pkt->pkt_field      = (self->oavctx->field_order > AV_FIELD_PROGRESSIVE);
    pkt->pkt_aspect_num = self->src_pkt->pkt_aspect_num;
    pkt->pkt_aspect_den = self->src_pkt->pkt_aspect_den;
    return 0;
}
